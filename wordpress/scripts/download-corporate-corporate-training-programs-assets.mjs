...
const html = fs.readFileSync(
  path.join(mirrorRoot, 'corporate-corporate-training-programs/index.html'),
  'utf8',
);
...
const cssPath = 'wp-content/uploads/elementor/css/post-20298c1dc.css';
paths.add(cssPath);

const cssLocal = path.resolve(__dirname, '../public', cssPath);
if (fs.existsSync(cssLocal)) {
  const css = fs.readFileSync(cssLocal, 'utf8');
  const cssPattern = /wp-content\/uploads\/[a-zA-Z0-9_./%-]+\.(png|jpe?g|gif|webp|svg)/gi;
  let match = cssPattern.exec(css);
  while (match) {
    paths.add(match[0].split('?')[0]);
    match = cssPattern.exec(css);
  }
