...
const html = fs.readFileSync(path.join(mirrorRoot, 'testimonials/index.html'), 'utf8');
...
if (fs.existsSync(cssPath)) {
  ...
  const bgMatches = css.matchAll(/url\(["']?\.\.\/\.\.\/([^"')]+)["']?\)/g);
  for (const match of bgMatches) {
    const rel = `wp-content/uploads/${match[1]}`;
    if (IMAGE_EXT.test(rel)) paths.add(rel);
  }
}

['wp-content/uploads/elementor/css/post-17365c9b9.css'].forEach((file) => paths.add(file));
...
console.log(
  `Testimonials assets: ${ok} downloaded, ${mirror} from mirror, ${skip} skipped, ${fail} failed (${list.length} unique paths)`,
);
