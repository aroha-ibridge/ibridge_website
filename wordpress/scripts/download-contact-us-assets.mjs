...
const html = fs.readFileSync(path.join(mirrorRoot, 'contact-us/index.html'), 'utf8');
...
['wp-content/uploads/elementor/css/post-292e44.css'].forEach((file) => paths.add(file));
...
console.log(
  `Contact Us assets: ${ok} downloaded, ${mirror} from mirror, ${skip} skipped, ${fail} failed (${list.length} unique paths)`,
);
