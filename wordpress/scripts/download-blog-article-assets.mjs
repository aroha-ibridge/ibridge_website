...
if (!slug) {
  console.error('Usage: node scripts/download-blog-article-assets.mjs <slug>');
  process.exit(1);
}

const htmlPath = path.join(mirrorRoot, slug, 'index.html');
if (!fs.existsSync(htmlPath)) {
  console.error('Mirror HTML not found:', htmlPath);
  process.exit(1);
}

const html = fs.readFileSync(htmlPath, 'utf8');

const ASSET_EXT = /\.(png|jpe?g|gif|webp|svg|mp4|ico|jpg|pdf)(\?|$)/i;
const paths = new Set();

const patterns = [
  /(?:\.\.\/)?wp-content\/uploads\/[a-zA-Z0-9_./%-]+\.(png|jpe?g|gif|webp|svg|mp4|ico|jpg|pdf)/gi,
  /https?:\/\/ibridge360\.com\/wp-content\/uploads\/[a-zA-Z0-9_./%-]+\.(png|jpe?g|gif|webp|svg|mp4|ico|jpg|pdf)/gi,
];
...
const cssMatch = html.match(/elementor\/css\/(post-16187[a-z0-9]+\.css)/i);
if (cssMatch) {
  paths.add(`wp-content/uploads/elementor/css/${cssMatch[1]}`);
}

const cssPath = cssMatch ? `wp-content/uploads/elementor/css/${cssMatch[1]}` : null;
if (cssPath) {
  const cssLocal = path.resolve(__dirname, '../public', cssPath);
  if (fs.existsSync(cssLocal)) {
    const css = fs.readFileSync(cssLocal, 'utf8');
    const relPattern = /\.\.\/\.\.\/([0-9]{4}\/[0-9]{2}\/[^)"']+\.(png|jpe?g|gif|webp|svg|pdf))/gi;
