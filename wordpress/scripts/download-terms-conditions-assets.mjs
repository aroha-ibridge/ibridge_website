...
const html = fs.readFileSync(path.join(mirrorRoot, 'terms-conditions/index.html'), 'utf8');
...
['wp-content/uploads/elementor/css/post-134fa74.css'].forEach((file) => paths.add(file));
...
console.log(
  `Terms & Conditions assets: ${ok} downloaded, ${mirror} from mirror, ${skip} skipped, ${fail} failed (${list.length} unique paths)`,
);
